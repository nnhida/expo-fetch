...
import { addBerita, deleteBerita, editBerita } from "@/app/api/berita"

interface uiProps {
    beritaList: beritaProps[] | undefined
}

export default function Ui({ beritaList}: uiProps) {

    const [open, setOpen] = useState<boolean>(false)
    const [typeForm, setTypeForm] = useState<string>('')

    const [beritaID, setberitaID] = useState<string>('')
    const [title, setTitle] = useState<string>('')
    const [desc, setDesc] = useState<string>('')
    const [image, setImage] = useState<string>('')


    function openForm(e: React.MouseEvent<HTMLButtonElement>, typeForm: string, data: beritaProps | null) {
        e.preventDefault()
        setTypeForm(typeForm)
        if (typeForm === 'edit') {
            setberitaID(data?.beritaID || '')
            setTitle(data?.title || '')
            setDesc(data?.desc || '')
            setImage(data?.image || '')
        } else {
    ...
    async function Submit(e: React.MouseEvent<HTMLButtonElement>) {
        ...
        if (beritaID) formData.append('beritaID', beritaID)
        if (title) formData.append('title', title)
        if (desc) formData.append('desc', desc)
        if (image) formData.append('image', image)

        if (typeForm === 'edit') {
            const result = await editBerita(formData)
            if (result.error) {
            ...
            const result = await addBerita(formData)
            if (result.error) {
                toast.error(result.error)
            } else {
                toast.success(result.success)
    ...
    async function delPost(e: React.MouseEvent<HTMLButtonElement>, postID: string) {
        ...
        const result = await deleteBerita(postID)
        if (result.error) {
            toast.error(result.error)
        } else {
            toast.success(result.success)
    ...
    return (
        <div className="min-h-screen">
            <section className='flex-col space-y-5 p-10 pt-40'>
                <div className=' flex justify-between'>
                    <p className='text-4xl font-bold'>Daftar Berita</p>
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {beritaList !== undefined && beritaList.map((item, index) => (
                            <tr key={index} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                ...
                                <td className="px-6 py-4 flex space-x-5">
                                    <button onClick={(e) => delPost(e, item.beritaID)} className="font-medium py-1 px-3 bg-red-500 text-white rounded-md">
                                        Delete
                                    </button>
